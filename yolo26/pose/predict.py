...
class Predictor(BasePredictor):
    """YOLO26 pose estimation predictor for Replicate deployment with selectable model size (n/s/m/l/x)."""

    def setup(self) -> None:
        """Load the default YOLO26 pose estimation model into memory."""
        self.model_size = "n"
        self.model = YOLO(f"yolo26{self.model_size}-pose.pt")
    ...
    def re_init_model(self, model_size: str) -> None:
        ...
        if model_size != self.model_size:
            self.model = YOLO(f"yolo26{model_size}-pose.pt")
    ...
    def predict(
        ...
        return_json: bool = Input(description="Return keypoint detections as JSON", default=False),
    ) -> Output:
