...
class Predictor(BasePredictor):
    """YOLO26 instance segmentation predictor for Replicate deployment with selectable model size (n/s/m/l/x)."""

    def setup(self) -> None:
        """Load the default YOLO26 instance segmentation model into memory."""
        self.model_size = "n"
    ...
    def re_init_model(self, model_size: str) -> None:
        ...
        if model_size != self.model_size:
            self.model = YOLO(f"yolo26{model_size}-seg.pt")
    ...
    def predict(
        ...
        return_json: bool = Input(description="Return segmentation results (boxes + masks) as JSON", default=False),
    ) -> Output:
