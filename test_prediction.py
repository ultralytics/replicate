    ...
    try:
        ...
        if not onnx_files:
            ...
            if model_dir == "yolo11n":
                print(f"💡 Run: python export_models.py --model yolo11n.pt --output yolo11n")
            else:
                print(f"💡 Run: python export_models.py --model best.pt --output custom")
            return
        ...
        print(f"💡 Make sure you have ultralytics installed: pip install ultralytics")
...
def main():
    ...
    parser.add_argument("--model", required=True, choices=["yolo11n", "custom"], 
                       help="Model directory to test")
